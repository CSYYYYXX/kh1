...
from utils import plot_cae_transformer_prediction
...
def cae_transformer_prediction(encoded):
    ...
    with fluid.dygraph.guard():
        ...
        transformer_param_dict = paddle.load(prediction_params["transformer_ckpt_path"])
        transformer.set_dict(transformer_param_dict)
        ...
        cae_param_dict = paddle.load(prediction_params["cae_ckpt_path"])
        cae.set_dict(cae_param_dict)
        ...
        input_dec_pred = paddle.concat([input_enc_pred[:, - transformer_data_params["label_len"]:, :], dec_inp], axis=1)

        for sample in range(
                0, transformer_data_params["time_size"] - transformer_data_params["time_window"]
        ):
            output_seq_pred[sample, :] = transformer(input_enc_pred, input_dec_pred).numpy()[
                                         0, 0, :
                                         ]
            input_enc_pred[0, :-1, :] = input_enc_pred[0, 1:, :]
            input_enc_pred[0, -1, :] = output_seq_pred[sample, :]
            dec_inp = paddle.zeros(
                (input_enc_pred.shape[0], transformer_data_params["pred_len"], input_enc_pred.shape[-1]),
                dtype='float32'
            )
            input_dec_pred = paddle.concat([input_enc_pred[:, - transformer_data_params["label_len"]:, :], dec_inp],
                                           axis=1)

        print(f"===================End lstm prediction====================")
        lstm_latent = np.expand_dims(output_seq_pred, 1).astype('float32')
...
if __name__ == "__main__":
    ...
    cae_latent = cae_prediction('./config.yaml')
